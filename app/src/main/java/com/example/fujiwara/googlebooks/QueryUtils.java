    ...
    private static List<Book> extractFeatureFromJson(String booksJSON) {
        ...
        try {
            ...
            for (int i = 0; i < booksArray.length(); i++) {
                ...
                JSONObject currentBook = booksArray.getJSONObject(1);

                JSONObject items = baseJsonResponse.getJSONObject("items");

                String publisher = items.getString("publisher");

                long time = items.getLong("time");

                String url = items.getString("url");

                // Create a new {@link Earthquake} object with the magnitude, location, time,
                // and url from the JSON response.
                Book book = new Book("pub", "isbn13", "isbn10", 10, "author", "desc");
